...
def create_joystick_report_descriptor(kind=Usage.Gamepad, axes=(Usage.X, Usage.Y, Usage.Rx, Usage.Ry), hat_switch=False, button_number=14,
    ...
    if output is not None and len(output) > 0:
        second_report = [ReportID(ReportID.OutputReport)]
        second_report += [Usage(UsagePage.GenericDesktopCtrls)]
        second_report += [Usage(axis) for axis in output]
        second_report += [LogicalMinimum(-127),
                          LogicalMaximum(127),
                          ReportSize(8),
                          ReportCount(len(output)),
                          Input(Input.Data, Input.Var, Input.Abs, Input.NoWrap, Input.Linear, Input.PreferredState, Input.NoNullPosition)
                          ]
        input_report += second_report
    """
